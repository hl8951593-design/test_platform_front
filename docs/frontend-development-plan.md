...
- 新增 Agent Runtime 前端工作台：`/agents` 路由、`src/types/agents.ts` 契约类型、`src/api/agents.ts` 普通 API 封装、`src/api/agentStream.ts` SSE fetch parser、本地 conversation/run history、创建 Agent Run、Codex 式三栏工作线程、右侧 Run/Tool/Approval/Memory/Runbook/Dashboard Inspector、ToolCall 详情 hydration、approval CAS 提交、Migration/Context/Loop/Memory 二级资源刷新、Runbook safe actions 到 `resume`、`reconcile`、`tool_call_detail` 的前端映射、SSE 断线自动重连、snapshot 补拉、terminal summary 校准、actions 主按钮、conversation transcript 校准、服务端 conversation export 下载、本地 export 兜底、history 搜索/重命名/置顶/删除、快捷键和 readiness checks 展示。中央线程已把用户可见 assistant 内容和 Agent loop 审计事件分离：`model.delta`/`model.markdown_normalized`/`model.completed`/`run.completed.result.message` 进入 GitHub Flavored Markdown 回复，工具请求修复、required tool 修复、history compacted 和 ToolCall 审计进入 timeline、ToolCall 或 Inspector。后端 `item_id` 作为 run/event/tool/approval/migration/context/action/debug/export 稳定 identity 进入 key、timeline/debug 定位和导出数据，SSE cursor 仍只使用 run-scoped `event_seq`/`Last-Event-ID`。后端已提供服务端 conversation/run list 契约；当前页面仍以本地 history MVP 为主，打开已有 conversation 时用服务端 transcript 校准，服务端列表接入列为下一阶段计划。
...
- 已完成 `/agents/runs` 创建、Run 详情、SSE、snapshot 补拉、terminal summary 校准、actions 主按钮、conversation transcript、conversation export、ToolCall、Approval CAS、Migration、ContextBuild、LoopObservation、Memory usage、Runbook、Dashboard、release gate promotion、`GET /agents/capabilities` 公开 ToolSpec identity 和 `GET /agents/skills` 元数据边界的前端接入与核心测试覆盖。
...
- 接入后端已提供的 `GET /agents/conversations` 和 `GET /agents/conversations/{conversation_id}/runs`，把本地 history index 升级为服务端历史视图；保留本地 index 作为离线/失败兜底。
