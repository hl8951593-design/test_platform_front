...
- 节点内部固定按前置、测试用例、后置执行；节点整体可上下移动。画布展示时主测试用例保持主卡片层级，前置/后置动作收进主测试用例卡片底部的分组摘要；分组默认收起，只展示动作数量和状态摘要，用户展开后在主卡片内部滚动管理。
...
响应取值的 `path` 支持引用执行位置之前已经声明的动作输出或响应取值，例如
`data.items.{{rowIndex}}.id`。单步调试展示取值结果时，前端会先用最近整场运行中的真实
前置输出、固定值动作的已配置值、脚本调试输出或上游调试取值解析路径模板，再从本次调试
响应中取值。若路径变量没有可用真实值，检查器显示“路径变量未解析”，不会把
`{{变量名}}` 当作字面 JSON 路径误报。
...
- 画布连线只表达测试用例节点到测试用例节点的执行顺序；节点内部的前置动作、后置动作、响应取值、变量引用和工具信息不绘制内部连线，只作为主测试用例卡片内的分组摘要、缩进卡片或可折叠详情展示。
- 前置/后置动作分组标题必须展示动作数量和通过、失败、执行中、未执行等摘要状态；分组默认收起，展开后动作列表在主测试用例卡片内部滚动，避免节点被前后置动作撑成多个横向大区块。
