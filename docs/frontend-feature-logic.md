...
当前支持按项目新建、编辑、复制和软删除场景；引用 HTTP 与 WebSocket 测试用例；加入条件和等待步骤；调整步骤顺序与失败策略；维护多组数据驱动变量；执行后端真实调试并展示步骤结果。

场景更新携带当前 `version`，执行接口支持环境、数据集和幂等键。完整数据模型和后端运行规则见 `scenario-composer-architecture.md`。
