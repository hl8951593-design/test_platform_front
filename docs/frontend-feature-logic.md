...
- 顶部统计使用紧凑可点击卡片，不占用主要编排空间。
- 三栏区域各自滚动，中间画布优先获得可用宽度；右侧配置标题在滚动时保持上下文。
- 步骤卡使用状态色条和轻量阴影表达选中、运行、通过、失败和跳过。
- 请求、断言、调试响应和响应取值保持原有功能与字段，只通过分组卡片降低视觉噪音。
...
| 功能 | 当前实现 |
| --- | --- |
| 缺陷列表 | 从 `/defects?project_id={projectId}` 加载，支持搜索、状态筛选、紧急程度筛选和分页 |
| 新建与编辑 | 维护 Bug 标题、指派人、Bug 类型、紧急程度、当前状态和 Bug 内容；标题和内容使用红色必填标记 |
| 富文本内容 | 使用富文本编辑框维护 `content_html`，支持加粗、斜体、列表、插入图片和粘贴图片 |
| 状态流转 | 列表卡片根据当前状态展示下一步流程按钮，调用 `/defects/{id}/status?project_id={projectId}` |
| 删除缺陷 | 二次确认后调用 `/defects/{id}?project_id={projectId}` 删除 |

缺陷状态包括：新创建、已激活、已确认、已修复、已验证、已关闭和重新激活。前端流转按钮表达推荐流程，最终合法性由后端接口校验。

富文本内容在前端提交和预览前会做基础清洗，移除脚本、事件属性和不安全 URL；服务端仍需进行最终 HTML 清洗、附件落库和权限校验。完整模块说明见 `defect-tracking-architecture.md`，接口目标契约见 `../api_docs/api_defects.md`。
