...
本文档记录 TestAuto 前端当前已经实现的核心功能逻辑，作为后续开发、联调和排查问题时的依据。代码以 `src/App.tsx`、`src/api/`、`src/pages/PlansPage.tsx`、`src/pages/ApiPage.tsx`、`src/pages/EnvironmentConfigsPage.tsx` 为主。
...
当前后端尚未提供测试计划接口，因此计划定义和计划执行历史按项目保存在浏览器本地。后续接入后端时，应保持页面只调用计划仓储/API 层，不在页面中直接拼接请求。
...
| 执行资产 | 并行加载当前项目 HTTP 用例、WebSocket 用例和可视化流程，可排序和移除 |
| 执行配置 | 多环境绑定、串行/并行、失败停止/继续、重试、超时、通知邮箱和标签 |
| 触发方式 | 手动触发、Cron 定时、Webhook 配置 |
| 调度日历 | 展示未来 14 天内已启用 Cron 计划，支持常用固定值、通配符、步长、范围和列表日期字段 |
| 手动运行 | 选择计划绑定环境后生成前端模拟执行记录，并更新计划最近执行时间；当前不会真实执行资产 |
...
| key | 内容 |
| --- | --- |
| `testauto_plans_project_{projectId}` | 当前项目测试计划定义 |
| `testauto_plan_runs_project_{projectId}` | 当前项目计划执行历史，最多保留 200 条 |

测试计划必须至少绑定一个环境和一个执行资产才能保存。切换项目后会重新读取对应项目计划，禁止跨项目复用本地计划数据。

测试计划完整架构、数据模型、模拟运行规则、Cron 能力边界和建议后端契约见 `test-plan-architecture.md`。

## 场景组合页

页面入口为 `/scenarios`，主文件为 `src/pages/ScenariosPage.tsx`，前端数据边界封装在 `src/api/scenarios.ts`。

当前支持按项目新建、编辑、复制和删除场景；引用 HTTP 与 WebSocket 测试用例；加入条件和等待步骤；调整步骤顺序与失败策略；维护多组数据驱动变量；执行前端模拟调试并展示步骤结果。

当前后端尚未提供场景接口，场景与调试历史保存在浏览器本地。完整数据模型、模拟运行规则和建议后端接口见 `scenario-composer-architecture.md`。
