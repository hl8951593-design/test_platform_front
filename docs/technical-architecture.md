...
| 测试计划 | `/plans` | 前端展示 | 计划统计、计划列表、计划操作入口 | `planStats`、`plans` |
| 可视化编排 | `/flow` | 前端展示 | 流程节点编排、节点配置、流程试运行入口 | 页面内静态节点 |
| 场景组合 | `/scenarios` | 前端展示 | 业务场景步骤组合、保存和运行入口 | `scenarioSteps` |
| 接口测试用例 | `/api` | 已接真实接口 | 统一维护 HTTP、WebSocket 用例定义、连接/请求数据、断言和示例响应，为自动化测试提供接口数据源 | `/test-cases`、`/websocket-test-cases` |
| 执行中心 | `/executions` | 前端展示 | 执行队列、执行状态、失败任务终止入口 | `executionRows` |
| 测试报告 | `/reports` | 前端展示 | 报告指标、失败聚类、AI 分析与补充用例入口 | 页面内静态数据 |
| 环境配置 | 顶部环境下拉、导航底部按钮 | 部分接真实接口 | 按项目查询环境，供接口测试用例保存和执行时关联 | `/projects/{project_id}/environments` |
...
| 示例响应维护 | 保存示例响应，便于用例设计、Mock、调试和后续编排 |
...
5. 新增/编辑弹窗用于维护用例名称、所属集合、维护人、状态、接口方法、URL、Params、Headers、Body、断言规则和示例响应。
...
| 顶部环境下拉 | 数据来自 `/projects/{project_id}/environments`，跟随当前项目切换 |
...
| `/projects/{project_id}/environments` | `GET` | 查询当前项目环境，顶部环境下拉使用 |
...
| 示例响应 | 维护示例响应，用于 Mock、编排和断言设计参考 | `exampleResponse` |
...
| `api_case:update` | 编辑接口定义、参数、请求体、断言和示例响应 |
...
当前项目尚未实现统一带 token 的请求客户端。新增受保护接口时，应优先扩展 `src/api/`，避免在页面组件里直接拼接鉴权头。
...
当前页面使用前端 mock 数据，还没有真实数据权限控制。根据平台业务，建议后续采用以下数据范围：
...
| `/plans` | 测试计划 | 是 | `plan:view`、`plan:create`、`plan:update` |
