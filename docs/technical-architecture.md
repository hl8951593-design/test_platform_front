...
| `docs/test-plan-architecture.md` | 测试计划前端架构、本地持久化、能力边界和后端接入建议 |
...
| 测试计划 | `/plans` | 前端本地闭环，待接后端 | 按项目维护计划、选择执行资产、配置调度、模拟运行、查看历史、导入导出 | 浏览器本地计划仓储、真实用例与流程资产 |
| 可视化编排 | `/flow` | 已接真实接口 | 流程节点编排、节点配置、保存、导入导出和流程试运行 | `/flows`、HTTP/WebSocket 用例资产 |
| 场景组合 | `/scenarios` | 前端本地闭环 | 按项目组合 HTTP/WebSocket 用例、条件和等待步骤，维护数据集并模拟调试 | `src/api/scenarios.ts` |
...
当前后端接口通过项目 ID 和登录用户进行部分数据隔离，但前端尚未接入完整角色、菜单、操作和字段权限。测试计划当前仅通过按项目划分的浏览器本地存储进行前端隔离。建议后续采用以下数据范围：
...
- 项目是主要的数据隔离边界。
- 测试计划负责组织执行批次；当前仅完成前端本地闭环，模拟运行不代表真实执行结果。
...
| 2026-06-08 | 完成测试计划前端本地闭环并新增独立技术文档，明确模拟运行、Cron 预览和后端接入边界 | 测试计划、文档 |
