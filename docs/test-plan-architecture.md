...
- 配置手动、Cron 和 Webhook 触发；Cron 模式支持选择每天执行时间并自动生成表达式，也保留表达式手动编辑入口。
- 配置时区、串行/并行、失败策略、重试、超时和通知邮箱。
- 查询后端生成的未来调度实例。
- 创建手动运行并查看、删除或清空运行历史。
- 调用后端接口导入和导出计划。

计划编辑器使用任务型弹窗：顶部展示计划名称、说明和环境/目标/触发摘要；主体为左侧基础信息、执行环境、执行目标，右侧固定展示已选执行顺序和触发执行策略；底部操作区固定显示取消和保存。弹窗在宽屏保持双栏，在窄桌面降级为单栏，内容区独立滚动且不允许页面级横向滚动。Cron 定时在前端用时间选择器生成每天执行的五段表达式，例如 `09:30` 生成 `30 9 * * *`；用户仍可直接编辑表达式。弹窗只改变前端布局和交互密度，不改变计划保存接口、字段或后端校验边界。
